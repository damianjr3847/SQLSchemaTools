...
const queryTableCheckConstraint: string =
    ...
    FROM (WITH base AS (
        ...
        WHERE
            ...
            AND (kcu.table_schema = 'public' OR kcu.table_schema IS NULL)
            AND (kcu2.table_schema = 'public' OR kcu2.table_schema IS NULL)    -- IF NO REFERNCED COLUMN EXISTS. ie. Unique Index etc.
        ...
        --SELECT * FROM table_constraints           -- SELECT * FROM information_schema.table_constraints) CC
     {FILTER_OBJECT} `;
...
//                    console.log(aux);
    ...
    async extractMetadataTables(objectName: string, aRetYaml: boolean = false, openTr: boolean = true): Promise<any> {
        ...
        try {
            ...
            rQuery      = await this.pgDb.query(this.analyzeQuery(queryTablesView, objectName, GlobalTypes.ArrayobjectType[2]), []);
            rTables     = rQuery.rows;
            rQuery      = await this.pgDb.query(this.analyzeQuery(queryTablesViewFields, objectName, GlobalTypes.ArrayobjectType[2]), []);
            rFields     = rQuery.rows;
