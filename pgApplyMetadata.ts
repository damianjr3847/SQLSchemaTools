...
export class pgApplyMetadata {   
    ...
    private pgStructure = require('pg-structure');
...
*/
    ...
    public async applyYalm(ahostName:string, aportNumber:number, adatabase:string, adbUser:string, adbPassword:string, objectType:string, objectName:string)  {                
        ...
        try {
            ...
            dbStructure= await this.pgStructure({database: adatabase, user: adbUser, password: adbPassword, host:ahostName, port:aportNumber}, ['public']);
            
            
            var tables = dbStructure.schemas.get('public').tables;  
            ...
            //get('public.account').columns.get('is_active').type 
